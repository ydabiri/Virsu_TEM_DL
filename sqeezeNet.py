...
#######################################################################################
squeeze = torch.hub.load('pytorch/vision:v0.5.0', 'squeezenet1_0', pretrained=True)
squeeze.class_to_idx = image_datasets.class_to_idx
#######################################################################################
for param in squeeze.parameters():
	    param.requires_grad = False
num_ftrs = 512
squeeze.classifier._modules["1"] = nn.Conv2d(512, numberOfClasses, kernel_size=(1, 1))
squeeze.num_classes = numberOfClasses
modelBeforeTraining = squeeze
########################################################################################
...
dataloadersVal = torch.utils.data.DataLoader(imageDatasetsVal, numberOfBatch, shuffle=True)
for inputs, labels in dataloadersVal:
    ...
    label_to_classNumber = {label: classNumber for classNumber, label in model.class_to_idx.items()}
    fcOut  = model.forward(inputs)
    softmax = nn.LogSoftmax(dim=1)
    probabilities = softmax(fcOut)
